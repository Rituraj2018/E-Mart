...
export const fetchProducts = (queryString = "") => async (dispatch) => {
    try {
        ...
        // 1. Parse the query string to check for category or keyword
        const params = new URLSearchParams(queryString);
        const categoryId = params.get("category");
        const keyword = params.get("keyword");

        // 2. Determine the correct backend endpoint
        let endpoint = "/public/products"; // Default endpoint
        
        if (keyword) {
            // If searching by keyword
            endpoint = `/public/products/keyword/${keyword}`;
        } else if (categoryId && categoryId !== "all") {
            // If filtering by category
            endpoint = `/public/categories/${categoryId}/products`;
        }
...
export const placeCODOrder = (sendData, toast, navigate, setLoading) => async (dispatch) => {
    try {
        setLoading(true);
        // Hitting your existing backend route, passing 'COD' as the paymentMethod param
        const { data } = await api.post("/order/users/payments/COD", sendData);
        
        if (data) {
            localStorage.removeItem("CHECKOUT_ADDRESS");
            localStorage.removeItem("cartItems");
            dispatch({ type: "CLEAR_CART"}); // Make sure CLEAR_CART exists in your cartReducer
...
export const addNewProductFromDashboard = 
    (sendData, toast, reset, setLoader, setOpen, isAdmin) => async(dispatch, getState) => {
        try {
            ...
            reset();
            setOpen(false);
            await dispatch(dashboardProductsAction("", isAdmin)); // pass isAdmin here
        } catch (error) {
            console.error("PRODUCT ERROR:", error?.response?.data); // shows real error
            toast.error(
...
export const updateProductImageFromDashboard = 
    ...
    try {
        ...
        // ADD these headers to override the default application/json
...
export const getAllUsersDashboard = (queryString = "") => async (dispatch) => {
  try {
    dispatch({ type: "IS_FETCHING" });
    const { data } = await api.get(`/auth/users?${queryString}`);
    dispatch({
      type: "GET_USERS",
      payload: data["content"],
      pageNumber: data["pageNumber"],
    ...
    dispatch({
      type: "IS_ERROR",
      payload: error?.response?.data?.message || "Failed to fetch users data",
    });
  }
};